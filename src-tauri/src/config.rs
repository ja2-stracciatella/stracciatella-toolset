...
impl PartialToolsetConfig {
    pub fn guess() -> PartialToolsetConfig {
        ...
        toolset_config.stracciatella_home = stracciatella_home.as_ref().map(|v| v.to_string_lossy().to_string());
        if let Some(stracciatella_home) = stracciatella_home {
            ...
            toolset_config.vanilla_game_dir = engine_options.as_ref().map(|v| v.vanilla_game_dir.to_string_lossy().to_string());
        }
...
impl ToolsetConfig {
    pub fn path() -> Option<PathBuf> {
        let project_dirs = directories::ProjectDirs::from("io", "ja2-stracciatella", "stracciatella-toolset")?;
    ...
    pub fn to_engine_options(&self) -> EngineOptions {
        ...
        engine_options.stracciatella_home = (&self.stracciatella_home).into();
        engine_options.assets_dir = (&self.stracciatella_install_dir).into();
        engine_options.vanilla_game_dir = (&self.vanilla_game_dir).into();
        engine_options
    }
}